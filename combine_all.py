...
train2 = []
#with open("./KBP-SF48-master/train_sf.txt", "rb") as f:  
with open(train_file+"/validate_sf.txt", "rb") as f:
    for l in f:
        line = l.decode().split("\t")
        tmp = len(line[1].split(" ")) 
        train2.append(line[1])
        test2.append(line[0])
...
def word2idx_array_valid(sentence_list, length, word2idx):
...
v = word2idx_array_valid(train2, len_max, word2idx)
t = word2idx_array_valid(train3, len_max, word2idx)
y = torch.LongTensor(np.asarray(test, dtype=np.float32))
vy = torch.LongTensor(np.asarray(test2, dtype=np.float32))
ty = torch.LongTensor(np.asarray(test3, dtype=np.float32))
...
v2 = char2idx_array(train2, len_max, char2idx, 9)
t2 = char2idx_array(train3, len_max, char2idx, 9)
...
train_dataset = Data.TensorDataset(torch.LongTensor(x), y, torch.LongTensor(x2))
train_dataset = Data.TensorDataset(torch.LongTensor(v), vy, torch.LongTensor(v2))
train_dataset = Data.TensorDataset(torch.LongTensor(t), ty, torch.LongTensor(t2))
...
def test(model2):
    ...
    with torch.no_grad():
        for batch_idx, (data, target, data2) in enumerate(test_loader):
            model2.zero_grad()
...
def train(model2, optimizer):
    ...
    for epoch in range(1, 100):
        ...
        test_loss, test_acc = eval(model2, file)
        file.write("\t"+str(val_loss.item())+"\t"+str(val_acc.item())+"\t"+str(test_loss.item())+"\t"+str(test_acc.item())+"\t"+str(epoch)+"\n")
